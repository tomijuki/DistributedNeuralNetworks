...
# Run the experiment N times with NO fault injection -- the control group for
# experiment2.sh. Same apply / wait / collect / teardown cycle, nothing else.
#
# Per iteration:
#   apply -> wait ready -> wait for "training done" -> collect to output-nofault/run-N
#   -> delete -> wait for pods to fully terminate
#
# Usage:
#   ./experiment1.sh                           # 5 runs, DDP
#   ./experiment1.sh 3                         # 3 runs
#   RUNS=5 MANIFEST=pipeline_statefulset.yaml WATCH_POD=mnist-pipeline-3 \
#     LABEL=app=mnist-pipeline ./experiment1.sh
...
DEST_ROOT="${DEST_ROOT:-$HOME/DIPLOMSKI/DistributedNeuralNetworks/output-nofault}"
